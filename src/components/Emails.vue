      ...
      <md-card-content>
        ...
        >
          ...
          <md-table-row slot="md-table-row" slot-scope="{ item }">
            ...
            <md-table-cell v-if='emailName == "pageSends"' md-label="Name" md-sort-by="name">
...
export default {
  ...
  computed: {
    ...
    queriedData() {
      ...
      // let duplicateFilter = Array.from(new Set(emails.map(email => email.Email)))
      //  .map(name => {
      //    return emails.find(email => email.Email === name)
      //  })
      //  debugger
  ...
  methods: {
    ...
    saveEmailsCSV() {
      let vm = this;
      let filteredEmails = this.getCorrectData();
    ...
    getCorrectData(){
      let filteredEmails;
      if (this.emailName == 'pageSends') {
        filteredEmails = this.queriedData.map(email => ({
      ...
      if (this.emailName == 'matchingEmails') {
        filteredEmails = this.queriedData.map(email => ({
          Created: moment(email.created).format("MM/DD/YYYY"),
          Email: email.email,
          Kiosk: email.name,
    ...
    getPageShares(){
      ...
      .get().then(function(snapshot){
        ...
        vm.pageSends = snapshot.docs.map(function(doc){
          let data = doc.data();
          data.id = doc.id;
          return data;
        });
